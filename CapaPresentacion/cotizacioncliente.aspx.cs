...
namespace CapaPresentacion
{
    ...
    {
        CotizacionNegocio CotizacionNego = new CotizacionNegocio();
        CotizacionEntidad CotizacionEnti = new CotizacionEntidad();
        ...
        {
            cliente = Request.QueryString["datos1"];
            razon = Request.QueryString["datos2"];
            Label1.Text = cliente;
            Label2.Text = razon;
            ...
            if ((OpcionEnti.tbValor == 0))
            {
                ...
                Response.Write("<script language=javascript>alert('Error : No Tienes Acceso - Cotizacion de Clientes Detalle');window.location.href ='menup.aspx';</script>");
            }
            ListarDatos();
        ...
        private void ListarDatos()
        {
            ...
            try
            {   //tab = CotizacionNego.CotizacionConsultar(cliente);
                grdClientes.DataSource = CotizacionNego.CotizacionConsultar(cliente);
        ...
        {
            OpcionEnti = OpcionNego.OpcionConsultar(Session["rusiausuario"].ToString(), "tcotizadet");
            if ((OpcionEnti.tbValor == 1))
            {
                //Response.Redirect("clientedetalle.aspx?datos1=" + this.grdClientes.Rows[grdClientes.SelectedIndex].Cells[5].Text + "&datos2=" + this.grdClientes.Rows[grdClientes.SelectedIndex].Cells[0].Text + "&datos3=" + this.grdClientes.Rows[grdClientes.SelectedIndex].Cells[4].Text);

                Response.Redirect("cotizaciondetcliente.aspx?datos1=" + this.grdClientes.Rows[grdClientes.SelectedIndex].Cells[5].Text 
                    + "&datos2=" + this.grdClientes.Rows[grdClientes.SelectedIndex].Cells[0].Text 
                    + "&datos3=" + this.grdClientes.Rows[grdClientes.SelectedIndex].Cells[4].Text);
            }
            else
            {
                Response.Write("<script language=javascript>alert('Error : No Tienes Acceso a Ver este Detalle - tcotizadet');</script>");
                Response.Redirect("menup.aspx");
           }
            //Response.Write("<script language=javascript>alert('Opción por Actualizar al 04/09/2018');</script>");
        }
