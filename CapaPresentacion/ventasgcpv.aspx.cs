...
namespace CapaPresentacion
{
    ...
    public partial class ventasgcpv : System.Web.UI.Page
    {
        VentasGCPVNegocio VentasGCPVNego = new VentasGCPVNegocio();
        VentasGCPVEntidad VentasGCPVEnti = new VentasGCPVEntidad();
          ...
          private void VentasGCPVListarPL()
        {
            try
            {
                GridPuntoyVendedor.DataSource = VentasGCPVNego.VentasGCPVListarCDatos(Punto, FechaI, FechaF);
        ...
        protected void GridPuntoyVendedor_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (this.GridPuntoyVendedor.Rows[GridPuntoyVendedor.SelectedIndex].Cells[0].Text == "401")
            {
            ...
            if (this.GridPuntoyVendedor.Rows[GridPuntoyVendedor.SelectedIndex].Cells[0].Text == "999")
            {

                OpcionEnti = OpcionNego.OpcionConsultar(Session["rusiausuario"].ToString(), "ovendedordet");
                if ((OpcionEnti.tbValor == 1))
                {
                    Response.Redirect("ventasgcpvdetalle.aspx?datos1=" + Convert.ToDateTime(Label1.Text.ToString()) + "&datos2=" + Convert.ToDateTime(Label2.Text.ToString()) + "&datos3=" + this.GridPuntoyVendedor.Rows[GridPuntoyVendedor.SelectedIndex].Cells[0].Text + "&datos4=" + this.GridPuntoyVendedor.Rows[GridPuntoyVendedor.SelectedIndex].Cells[2].Text + "&datos5=" + this.GridPuntoyVendedor.Rows[GridPuntoyVendedor.SelectedIndex].Cells[3].Text);
                }
                else
                {
                    Response.Write("<script language=javascript>alert('Error : No Tienes Acceso a Ver este Detalle - ovendedordet');</script>");
